...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { category: @build.category, done_date: @build.done_date, name: @build.name, price: @build.price, start_date: @build.start_date, user_id: @build.user_id } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { category: @build.category, done_date: @build.done_date, name: @build.name, price: @build.price, start_date: @build.start_date, user_id: @build.user_id } }
